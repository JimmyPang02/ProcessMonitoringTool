...
def normalize(*args):
    ...
    if len(args) == 2:
        ...
        print(X_new.shape, X_normal_mean.shape, X_normal_std.shape)
        X_new_center = (X_new - X_normal_mean) / X_normal_std
        print(X_new_center.shape)
...
def fit_SFA(train_X, alpha):
    ...
    Args:
        train_X (np.array): 训练数据
        alpha (float): 显著性水平

    Returns:
        MT2 (np.array): T2统计量
        MT2_e (np.array): T2e统计量
        MS2 (np.array): S2统计量
        MS2_e (np.array): S2e统计量
        S2_threshold (np.array): S2统计量阈值
        S2e_threshold (np.array): S2e统计量阈值
        T2_threshold (np.array): T2统计量阈值
        T2e_threshold (np.array): T2e统计量阈值
    """
    ...
    alpha = 0.01
    level = 1 - alpha
    S2_threshold = scipy.stats.f.ppf(
        level, M, N - M - 1) * M * (N ** 2 - 2 * N) / (N - 1) / (N - M - 1)
    S2e_threshold = scipy.stats.f.ppf(
        level, Me, N - Me - 1) * Me * (N ** 2 - 2 * N) / (N - 1) / (N - Me - 1)
...
def runSFA(X_train, X_test, alpha=0.01):

    X_train_, X_test_ = normalize(X_train, X_test)

    # 训练
    MT2, MT2_e, MS2, MS2_e, S2_threshold, S2e_threshold, T2_threshold, T2e_threshold = fit_SFA(
        X_train_, 0.01)

    # 测试
    test_T2, test_T2e, test_S2, test_S2e = test_SFA(
        X_test_, MT2, MT2_e, MS2, MS2_e)

    return test_T2, test_T2e, test_S2, test_S2e,\
        S2_threshold, S2e_threshold, T2_threshold, T2e_threshold

    # 监控结果可视化
    # visualization_SFA(test_T2, test_T2e, test_S2, test_S2e,
    #                   S2_threshold, S2e_threshold, T2_threshold, T2e_threshold)
